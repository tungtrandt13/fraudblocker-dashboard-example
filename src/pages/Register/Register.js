...
function Register({ auth, accounts, createUser, updateUser, updateUserAccount, getUserAccounts, fetchLatestAccount }) {
    ...
    const checkRegistrationStep = () => {
        if (auth.user && accounts.data) {
            ...
            } else if (Validation.userHasAllRequiredFields(auth.user, accounts.data, accounts.subscription)) {
    ...
    const onClickNextAccountCreation = async (planSelected, currency = "USD") => {
        ...
        try {
            ...
            if (!accounts.data || !accounts.data.initial_plan || planSelected.id !== accounts.data.initial_plan.id) {
