...
function CurrentUserBox({ user, accounts, history, activeDomain }) {
    ...
    const updateClicks = async () => {
        try {
            ...
            const result = await fetchSiteClicks(user.account_id, subscription.id, user.timezone);
    ...
    const renderPlanIcon = (subscription, planName) => {
        ...
        return (
            <div className={styles.planIcon}>
                <img src={isStarterPlan ? PlanIcon : CrownIcon} alt={isStarterPlan ? "Starter Plan" : "Pro Plan"} />
            </div>
        );
    };
    ...
    const totalClicks = subscription?.plan?.metadata?.clicks ? parseInt(subscription.plan.metadata.clicks, 10) : 0;
                ...
                <div className={styles.planBox}>
                    ...
                    <div className={styles.clicks}>
                        <div className={styles.clicksSection}>
                            <div
                                className={`${styles.clicksCount} ${clicksData.clicks > totalClicks ? styles.limit : ""}`}
                            >
                        ...
                        >
                            {clicksData.clicks <= totalClicks ? (
                                <>
                                    ...
                                    {planName.toLowerCase().includes("starter") ? "Upgrade Now To Pro" : "Upgrade Now"}
                                </>
                            ) : (
                                <>
                                    <span>!</span>
