...
function SubMenuNavItem({
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            const shouldExpand = item?.title?.toLowerCase() &&
                (item.title.toLowerCase().includes('setting') ||
                 item.title.toLowerCase().includes('advertising'));
    ...
    useEffect(() => {
        if (accounts?.data && activeDomain) {
            ...
            if (totalDomains > prevTotalDomains && 
    ...
    const renderSubNavItems = () => {
        ...
        return item.options.map(link => {
            ...
            const disabledForViewer = link.protected &&
        ...
        <div className={styles.navItem}>
            {item.route === '/logout' ? (
                <a
                    ...
                    className={isNavActive ? styles.navItemTitleActive : parentNavStyle}
                >
                ...
                <SubNavLink
                    isDisabled={item.protected && (
                        userRole === 'Viewer' || 
                        (item.blockForRoles && item.blockForRoles.includes(userRole))
                    )}
                    item={item}
                    location={location}
