...
const convertToUSD = (rates, amount, currency = 'USD', noRound = false) => {
    ...
    if (currency === 'CAN') {
        return noRound ?
            amount * (rates.USD / rates.CAD) :
            Math.round(amount * (rates.USD / rates.CAD));
    }
    return noRound ?
        amount * (rates.USD / rates[currency]) :
        Math.round(amount * (rates.USD / rates[currency]));
};
...
const getSubscriptionClicksAndDomains = subscription => {
    if (subscription.appSumoSubscription) {
        return {
            ...
            domains: subscription.appSumoSubscription.plan.metadata.domains === 'unlimited' ||
...
const getSingleSubscription = (accounts, accountId) => {
    ...
    let subscription =
        filteredSubscriptions.find(
            item => item.metadata.account_id === accountId && item.status === 'active'
    ...
    const {
        clicks,
        domains
    } = getSubscriptionClicksAndDomains(latestSubscription);

    return {
        ...latestSubscription,
        plan: {
            ...latestSubscription.plan,
            metadata: { ...latestSubscription.plan.metadata,
                clicks,
                domains
            }
        }
...
const setCookie = (name, value, days = 1) => {
    ...
    if (window.location.href.includes('localhost')) {
        ...
        document.cookie = days >= 1 ? `${name}=${value || ''}${expires}; Path=/; Domain=.fraudblocker.com` : `${name}=${value || ''}; Expires=Thu, 01 Jan 1970 00:00:01 GMT; Path=/; Domain=.fraudblocker.com`;
    }
};
